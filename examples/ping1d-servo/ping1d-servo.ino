...
static const uint32_t vSound = 343000; // mm/s
...
void setup() {
  ...
  ping.set_speed_of_sound(vSound);
  // Initialise servo position
  moveServo(minAngle, false);
}
...
void loop() {
  ...
  int maxAngle = Serial.readStringUntil('\n').toInt();
  // Response received - perform requested rotation (there and back)
  Serial.print("Rotating ");
  Serial.print(maxAngle);
  Serial.println(" degrees");

  for (int pos = minAngle; pos <= maxAngle; pos += increment) {
    moveServo(pos);
    readSonar();
  }

  for (int pos = maxAngle; pos >= minAngle; pos -= increment) {
    moveServo(pos);
    readSonar();
  }
}
